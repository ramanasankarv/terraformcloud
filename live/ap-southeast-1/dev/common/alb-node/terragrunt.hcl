...
inputs = {
  ...
  alb_name                = "${local.project}-${local.appname}-alb-node-${local.env}"
  sg_name                 = "${local.project}-${local.appname}-alb-node-sg-${local.env}"
  tg_name                 = "${local.project}-${local.appname}-node-tg-${local.env}"
  tags                    = local.tags
  vpc_id                  = dependency.vpc.outputs.vpc_id
  public_subnets          = dependency.vpc.outputs.public_subnets
  alb_acm_certificate_arn = local.alb_acm_certificate_arn
  route53_record_name     = "${local.env}.snapshot-albbmq2.com"
