...
inputs = {
  ...
  alb_name                = "${local.project}-${local.appname}-alb-api-${local.env}"
  sg_name                 = "${local.project}-${local.appname}-alb-api-sg-${local.env}"
  tg_name                 = "${local.project}-${local.appname}-api-tg-${local.env}"
  tags                    = local.tags
  vpc_id                  = dependency.vpc.outputs.vpc_id
  public_subnets          = dependency.vpc.outputs.public_subnets
  alb_acm_certificate_arn = local.alb_acm_certificate_arn
  route53_record_name     = "${local.env}-api.snapshot-albbmq2.com"
