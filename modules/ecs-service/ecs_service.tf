resource "aws_ecs_service" "api" {
  name            = "${var.project}-${var.appname}-ecs-service-cms-${var.env}"
  cluster         = var.ecs_cms_cluster_id
...
resource "aws_security_group" "ecs_task_sg" {
  name        = "${var.project}-${var.appname}-ecs-task-sg-${var.env}"
...
resource "aws_security_group_rule" "ingress_rds_rules" {
  type                     = "ingress"
  from_port                = 3306
  to_port                  = 3306
  protocol                 = "tcp"
