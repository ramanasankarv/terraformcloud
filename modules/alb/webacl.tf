resource "aws_wafv2_ip_set" "ipsets" {
  name               = "${var.project}-${var.appname}-webacl-ipsets-alb-${var.env}"
...
resource "aws_wafv2_web_acl" "alb_web_acl" {
  name        = "${var.project}-${var.appname}-webacl-alb-${var.env}"
  description = "Rule"
  scope       = "REGIONAL"
  ...
  rule {
    ...
    visibility_config {
      ...
      metric_name                = "${var.project}-${var.appname}-webacl-alb-${var.env}-rule"
  ...
  visibility_config {
    cloudwatch_metrics_enabled = false
    metric_name                = "${var.project}-${var.appname}-webacl-alb-${var.env}-metric"
    sampled_requests_enabled   = false
  }
